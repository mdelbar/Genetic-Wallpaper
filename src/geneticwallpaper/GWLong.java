...
public class GWLong {
    ...
    public GWLong(int popsize, int width, int height) {
    ...
    public Wallpaper[] generate() {
        ...
        int diffDivideThreshold = Math.max(1, numIterations / 10);
...
//        long maxFit = 8421504L*8L*((long)maxLookaround)*((long)w)*((long)h);
        int maxFit = w*h;
...
//        calculateFitnesses(pop);
    ...
    private int pixelFitness(Wallpaper wp, int x, int y) {
        ...
        for(int i = 1; i <= maxLookaround; i++) {
            ...
            int diffNW = pixelC ^ wp.pixels[minYLook][minXLook];
            int diffN = pixelC ^ wp.pixels[minYLook][x];
            int diffNE = pixelC ^ wp.pixels[minYLook][maxXLook];
            int diffW = pixelC ^ wp.pixels[y][minXLook];
            int diffE = pixelC ^ wp.pixels[y][maxXLook];
            int diffSW = pixelC ^ wp.pixels[maxYLook][minXLook];
            int diffS = pixelC ^ wp.pixels[maxYLook][x];
            int diffSE = pixelC ^ wp.pixels[maxYLook][maxXLook];
...
//            fitness += (fitnessValueNW <= maxDiff) ? (1) : (-1);
        ...
        return (fitness <= maxDiff*8*maxLookaround) ? (1) : (-1);
